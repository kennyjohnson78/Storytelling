...
export const routerTransition: AnimationEntryMetadata = trigger('routerTransition', [
  ...
  transition('detail => *', [
    group([
      query(':self', [
        ...
        animate(
          '0.5s ease-in-out',
          style({ top: 0, bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(255, 255, 255, 0)' })
        )
      ]),
      query(':leave', [animate('0.5s ease-in-out', style({ transform: 'scale3d(.5, .5, .5)', opacity: 0 }))], {
        optional: true
      })
    ])
  ])
